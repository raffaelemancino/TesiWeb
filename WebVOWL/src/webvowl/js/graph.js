...
//	console.log(app.jsonData);
	var graph = {},
		...
		language = "default",
		paused = false,
		storedLevelValue={ activity:0, resource:-1},
	...
	function myContextMenu(){
        var stile = "top=center, left=center, width=750, height=350, status=no, menubar=no, toolbar=no scrollbars=no",
            stileDel="top=center, left=center, width=400, height=100, status=no, menubar=no, toolbar=no scrollbars=no";
        ...
        fun: function () {
			...
			var _type=getType(dataJson, _clickedNode.id());
			console.log(_type);
			var _level=getLevel(dataJson, _clickedNode.id());
			//alert per eventuale nodo root
			if(_type==-1){
				alert("Cannot select a node parent of Activity or Resource");
			}else{
				storedLevelValue[_type]=_level;
				
			}
			console.log(storedLevelValue);
			}
		},{
        name: 'Unselect all',
        title: 'unselect button',
        fun: function () {			
			storedLevelValue["resource"]=-1;//questa e la seguente resettano il livello
			storedLevelValue["activity"]=0;
			}
		},{
        name: 'Send',
        title: 'send button',
		disable: true,
        fun: function () {
			/*invio i valori (se selezionati) a cnet2adweb*/
			localStorage.setItem("storedLevelValue", JSON.stringify(storedLevelValue) );
			//window.close();
			storedLevelValue["resource"]=-1;
			storedLevelValue["activity"]=0;
			console.log(storedLevelValue);
			}
		}];
	
/* 		if(storedLevelValue["resource"]!=0||storedLevelValue["activity"]!=0){
			$('#enableOption').click(function (e) {	
				var updateObj = [{
					name: 'Send',
					disable: false
				}];
				$('#'+_clickedNode.id()).contextMenu('update', updateObj);
			});
		} */
//Calling context menu
		...
		//mostra l'opzione send se almeno uno dei due livelli è selezionato
		if(storedLevelValue["resource"]>0||storedLevelValue["activity"]>0){
			var updateObj = [{
				name: 'Send',
				disable: false
...
function getLevel(json, param1) {

    var classAttribute = json.classAttribute;
    var last_node;
    var lengthMax = 0;
    var length = 0;
    var subClasses = getChildren(classAttribute, param1);

    while (subClasses !== undefined) {

        length = 1;
        last_node = subClasses[0];
        subClasses = getChildren(classAttribute, last_node);
        while (subClasses !== undefined) {
        ...
        var idx = getIndexFromID(classAttribute, last_node);
        var superClassID = classAttribute[idx].superClasses[0];
        idx = getIndexFromID(classAttribute, superClassID);
        classAttribute[idx].subClasses.splice(getIndexFromID(classAttribute[idx].subClasses, last_node), 1);
        if(classAttribute[idx].subClasses.length == 0) delete classAttribute[idx].subClasses;
        classAttribute.splice(getIndexFromID(classAttribute, last_node), 1);

        length = 0;
        subClasses = getChildren(classAttribute, param1);
    }
...
function getChildren(classAttribute, id) {

    var idx = getIndexFromID(classAttribute, id);
    var subClasses = classAttribute[idx].subClasses;

    return subClasses
}


function getType(json, id) {
    ...
    while(exit == 0){
        ...
        var idx = getIndexFromID(classAttribute, id);
        console.log(classAttribute[idx].iri);

        var iri = classAttribute[idx].iri;
        iri = iri.substring(iri.indexOf("#") + 1);

        if (iri.toLowerCase() === "resource") return "resource";
        if (iri.toLowerCase() === "activity") return "activity";

        var superClasses = classAttribute[idx].superClasses;
        if (superClasses === undefined) return -1;

        id = superClasses[0];
        idx = getIndexFromID(classAttribute, id);

        iri = classAttribute[idx].iri;
        iri = iri.substring(iri.indexOf("#") + 1);
...
function getIndexFromID(classAttribute, id)
{
    for(var i in classAttribute){
        if(classAttribute[i].id == id)
        {
            return i;
        }
    }
